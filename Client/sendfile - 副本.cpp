...
void Sendfile::readPendingDatagrams_send()
{
    while (udpSocketserver->hasPendingDatagrams()) {
        ...
        qDebug() << count_send << datagram; //打印收到的消息
        if("##filehead recved" == datagram) //
        {
            ...
            while(!filesend.atEnd() && num<N) {
                ...
                send_packages[num].append("&&"); //包尾
                isconfirmed[num] = false; //设置未确认
                num++;//0-7
        ...
        else if("##file recved"== datagram) //收到完全确认
        {
            ...
            if(sum_s >= 60)
            {
                ...
                sum_s = sum_s%60; //取余
                QMessageBox::warning(this,"传输完成",QString("共花费时间%1分钟%2秒").arg(sum_m).arg(sum_s),QMessageBox::Yes);
            }
            else
            {
                QMessageBox::warning(this,"传输完成",QString("共花费时间%1秒").arg(sum_s),QMessageBox::Yes);
            }
            timer->stop(); //计时器重置
            delete timer; //删除计数器
            ui->bt_exit->setEnabled(true);
...
void Sendfile::readPendingDatagrams_recv() //接收
{
    while (udpSocketclient->hasPendingDatagrams()) {
        ...
        {
            qDebug() << "收到请求";
            filenamerecv = QString(datagram).section("##",1,1);
            fileSizerecv = QString(datagram).section("##",2,2).toInt();
            ui->le_name->setText(filenamerecv);
        ...
        else//文件写入部分
        {
            if(!istail) //未写完还需要读入
            {
                ...
                QList<QString> l =  QString(datagram).split(QString("&&")); //取得消息列表
                qDebug() << "消息列表大小:" << l.size();
                for(int index=0; index<l.size(); index++) //最后一个为""
                {
                    QString buf = l[index]; //取到其中一部分
